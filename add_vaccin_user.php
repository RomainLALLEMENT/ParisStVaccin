			...
			//	$succes = verifOccurenceUserVaccin($idVaccin,$idUser);
      		      ...
      		      if($occurence === false){
      		          ...
      		          /*redirection vers la page modif info vaccins*/
										$idCarnet = getIdCarnetByIdUserandIdVaccin($idUser,$idVaccin);
      		          $errors['vaccin'] = 'Ce vaccin est déjà renseigné<br>
										<a href="modificationVaccinUser.php?id='.$idCarnet['id'].'">Cliquez ici pour modifier les  informations concernant ce vaccin</a>';
      		      }
				  }
	    }
	                        ...
	                        <select name="vaccin" id="vaccin">
	                            ...
	                            <?php foreach ($vaccins as $key => $vaccin) {
	                                ...
	                                <?php if(!empty($_POST['vaccin']) && $_POST['vaccin'] == $key ) { ?>
	                                    <option value="<?=  $key ?>"selected><?= $vaccin ?></option>
	                                <?php } else { ?>
	                                    <option value="<?=  $key+1 ?>"><?php echo $vaccin; ?></option>
	                                <?php } ?>
	                            <?php } ?>
	                        </select>
